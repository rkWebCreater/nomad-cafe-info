...
//--------  営業中かどうかの判定 スライダーのカードをコンポーネント化しているのでCafeCards.vue側にもcheckIfOpen関数を記述する :string :booleanはtypescript
...
//----検索バーの処理　検索・絞り込みロジック
// 第1引数に全データ、第2引数にキーワード、第3引数にエリア名を受け取る
const searchCafes = (allCafes: any[] , keyword: string , area : string ) =>{

  return allCafes.filter(cafe =>{

    // エリアが指定されていない(空文字)か、一致していればOK
    const matchArea = !area || cafe.areaId === area

    //キーワードが指定されていない(空文字)か、名前や住所に含まれていればOK
