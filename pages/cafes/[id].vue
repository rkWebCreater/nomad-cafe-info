...
const {allCafesData , checkIfOpen } = useCafe()
...
const {data : cafe } = await useAsyncData(
    ...
    () => {
        const found = allCafesData.value.find(cafe => cafe.id === cafeId.value)
...
const othersCafes = computed(() => {
  return allCafesData.value.filter(cafe => cafe.id !== cafeId.value && checkIfOpen(cafe.businessHours)=== true)
})
</script>
